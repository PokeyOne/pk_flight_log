...
fn test_new_writes_id() {
    ...
    let w = PacketWriter::new(&mut buf, TEST_PACKET_ID, 0x1234).unwrap();
...
fn test_writing_u8() {
    ...
    let mut w = PacketWriter::new(&mut buf, TEST_PACKET_ID, 0x1234).unwrap();
...
fn test_initialize_with_zero_length_buffer_returns_none() {
    ...
    let w = PacketWriter::new(&mut buf, TEST_PACKET_ID, 0x1234);
    assert!(w.is_none());
}
...
fn test_read_values_all_whole() {
    ...
    let ((id, timestamp), mut iterator) = r.read_values(TEST_PACKET_LIST_SLICE).unwrap();

    assert_eq!(id, TEST_PACKET_ID);
